...
const Navbar = ({setDark,dark}) => {
  ...
  className={`dark:bg-gray-800  flex box-border fixed top-0 left-0 right-0 bg-white shadow-md justify-between px-12 lg:px-32 py-6 ${
    shownav ? navActive?'flex-col gap-4':"flex-row items-center justify-between" : 'flex-row items-center justify-between'
...
>
      ...
      <div className={` gap-4  ${navActive? shownav?'flex flex-col':'hidden': 'flex'} `}>
      ...
      <div className='flex items-center fixed top-10'>
        <button className='fixed right-20 bg-5e3bee text-white p-2 rounded-md hover:scale-105 transition-all duration-500 ease-in-out'>Contact Me</button>
      <div onClick={changeTheme}className= 'fixed right-4 hover:scale-110 transition-all duration-500 ease-in-out'>
        {dark?
